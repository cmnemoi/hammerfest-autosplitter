...
differ by **when they start** and **when they stop**, and that is the whole
subject.
...
**So we stopped racing.** `GameMode.main()` returns on `fl_lock` *before* it
...
game with another cinematic, and a wrong correction costs the run where a
missing one costs one read.
...
So the LiveSplit *real time* starts at the `start()` call — that is, when the
search finishes, a few hundred milliseconds late. There is no way to correct
it.
...
it. It excludes pauses and level transitions, so it is not a run time — but it
is the number the player recognises.
